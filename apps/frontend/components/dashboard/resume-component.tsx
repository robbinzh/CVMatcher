...
import { PersonalInfo, ExperienceEntry, EducationEntry, ResumePreview } from '@/components/common/resume_previewer_context';

interface ResumeData extends ResumePreview {}
...
const Resume: React.FC<ResumeProps> = ({ resumeData }) => {
	...
	console.log('Resume Component - Experience data:', experience);
	console.log('Resume Component - Experience length:', experience?.length);
	
	// 检查经历数据的重复
	if (experience && experience.length > 0) {
		const uniqueIds = new Set(experience.map(exp => exp.id));
		if (uniqueIds.size !== experience.length) {
			console.warn('Resume Component - Found duplicate experience IDs:', experience);
		}
	}
	...
	return (
		...
		<div className="font-mono bg-gray-950 text-gray-300 p-6 shadow-lg rounded-lg w-full border border-gray-600">
			...
			{personalInfo && (
				<div className="text-center mb-4 text-md pb-6 border-gray-700">
					...
					<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 text-left px-2">
